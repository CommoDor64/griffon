...
#[cfg(test)]
mod tests {
    use crate::aes::{to_bytes, to_u128};

    #[test]
    fn test_from_to_u128() {
        let input: u128 = 0x0123_4567_89AB_CDEF_0123_4567_89AB_CDEF;
        assert_eq!(input, to_u128(to_bytes(input)));
    }
}
...
fn add_round_key(key: u128, input: u128) -> u128 {
    key ^ input
}

fn subbytes(state: u128) -> u128 {
    let bs = to_bytes(state);
    let mut res = [0u8; 16];
    for (i, b) in bs.iter().enumerate() {
        let col = 0x0F & b;
        let row = 0xF0 & b;
        res[i] = SBOX[row as usize][col as usize];
    }

    to_u128(res)
}

fn state_to_matrix(state: u128) -> [u32; 4] {
    let mut res = [0u32; 4];
    for i in 0..4 {
        res[i] = (state >> i * 32) as u32;
    }
    res
}

fn matrix_to_state(mat: [u32; 4]) -> u128 {
    let mut res = 0u128;
...
#[cfg(test)]
mod tests1 {
    use crate::aes::{matrix_to_state, state_to_matrix};

    #[test]
    fn test_state_to_matrix() {
        let input: u128 = 0x0123_4567_89AB_CDEF_0123_4567_89AB_CDEF;
        let res: [u32; 4] = [0x89ABCDEF, 0x01234567, 0x89ABCDEF, 0x01234567];

        assert_eq!(res, state_to_matrix(input));
    }

    #[test]
    fn test_from_to_matrix() {
        let input: u128 = 0x0123_4567_89AB_CDEF_0123_4567_89AB_CDEF;
        assert_eq!(input, matrix_to_state(state_to_matrix(input)));
    }
}
fn state_to_matrix_u8(state: u128) -> [[u8; 4]; 4] {
    let mut res = [[0u8; 4]; 4];
    let mut helper = [0u8; 4];
    for i in 0..16 {
        helper[i % 4] = (state >> i * 8) as u8;
...
fn matrix_to_state_u8(mat: [[u8; 4]; 4]) -> u128 {
    let mut res = 0u128;
    for (i, r) in mat.iter().enumerate() {
        for (j, c) in r.iter().enumerate() {
            res |= (*c as u128) << i * 32 + j * 8;
        }
    }
...
#[cfg(test)]
mod tests2 {
    use crate::aes::{matrix_to_state_u8, state_to_matrix_u8};
    ...
    fn test_state_to_matrix() {
        ...
        assert_eq!(res, state_to_matrix_u8(input));
    }

    #[test]
    fn test_from_to_matrix() {
        let input: u128 = 0x0123_4567_89AB_CDEF_0123_4567_89AB_CDEF;
        assert_eq!(input, matrix_to_state_u8(state_to_matrix_u8(input)));
    }
}

fn shift_rows(state: u128) -> u128 {
    let mut mat = state_to_matrix(state);
    mat[1] = mat[1].rotate_left(8);
    mat[2] = mat[2].rotate_left(16);
    mat[3] = mat[3].rotate_left(24);
